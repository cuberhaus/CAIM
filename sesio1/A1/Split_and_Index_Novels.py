...
def create_indexes():
	...
	for Dir in directory:
	    for i in range(0,3):
	        ftxt = codecs.open("./groups/"+str(Dir)+"/"+str(i)+str(Dir), "r", encoding='iso-8859-1')
	        text = ''
	        for line in ftxt:
	        	text += line
        # Insert operation for a document with fields' path' and 'text'
	        ldocs.append({'_op_type': 'index', '_index': str(i)+str(Dir), 'path': "./groups/"+str(Dir)+"/"+str(i)+str(Dir), 'text': text})
